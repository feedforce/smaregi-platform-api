  ...
  class UpdateStoreResponse
    ...
    def self.openapi_types
      {
        ...
        :'point_condition' => :'PointCondition',
        :'receipt_print_info' => :'UpdateStoreResponseReceiptPrintInfo'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UpdateStoreResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UpdateStoreResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [store_id, store_code, store_name, store_abbr, print_receipt_store_name, print_stock_receipt_store_name, division, post_code, address, phone_number, fax_number, mail_address, homepage, temp_tran_mail_address, price_change_flag, sell_division, sum_proc_division, sum_date_change_time, sum_ref_column, point_not_applicable, tax_free_division, max_bundle_product_count, max_discount_rate, carriage_display_flag, terminal_adjustment_cash_flag, terminal_check_cash_flag, waiter_adjustment_division, saving_auto_division, saving_auto_price, cancel_setting_division, rounding_division, discount_rounding_division, card_company_select_division, gift_receipt_valid_days, tax_label_normal, tax_label_reduce, display_sequence, face_payment_use_division, ins_date_time, upd_date_time, point_condition, receipt_print_info].hash
    end
