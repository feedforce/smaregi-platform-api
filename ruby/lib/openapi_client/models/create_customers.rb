  ...
  class CreateCustomers
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CreateCustomers` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreateCustomers`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @customer_code.to_s.length < 1
    ...
    def valid?
      ...
      return false if @customer_code.to_s.length < 1
    ...
    def customer_code=(customer_code)
      if customer_code.nil?
        fail ArgumentError, 'customer_code cannot be nil'
      end
    ...
    def hash
      [customer_code, customer_no, rank, staff_rank, first_name, last_name, first_kana, last_kana, post_code, address, phone_number, fax_number, mobile_number, mail_address, mail_address2, mail_address3, company_name, department_name, managerial_position, sex, birth_date, point_expire_date, entry_date, leave_date, point_giving_unit_price, point_giving_unit, pin_code, passport_no, nationality, alphabet_name, mail_receive_flag, note, note2, favorite_list, browsing_list, status, store_id].hash
    end
