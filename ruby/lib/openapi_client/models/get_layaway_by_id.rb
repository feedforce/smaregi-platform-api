  ...
  class GetLayawayById
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetLayawayById` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetLayawayById`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@details.nil? && @details.length < 1
        invalid_properties.push('invalid value for "details", number of items must be greater than or equal to 1.')
      end

      if !@customer_groups.nil? && @customer_groups.length < 1
        invalid_properties.push('invalid value for "customer_groups", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@details.nil? && @details.length < 1
      return false if !@customer_groups.nil? && @customer_groups.length < 1
      true
    end
    ...
    def details=(details)
      if !details.nil? && details.length < 1
        fail ArgumentError, 'invalid value for "details", number of items must be greater than or equal to 1.'
      end
    ...
    def customer_groups=(customer_groups)
      if !customer_groups.nil? && customer_groups.length < 1
        fail ArgumentError, 'invalid value for "customer_groups", number of items must be greater than or equal to 1.'
      end
