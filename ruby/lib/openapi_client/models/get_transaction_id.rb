...
module OpenapiClient
  class GetTransactionId
    ...
    #   string  合計ポイント：取引後のポイント 現在ポイント + 付与ポイント - 使用ポイント。 ※ ポイント値の直接入力は9桁まで可能です。ただし、ポイントの加算により9桁を超える値が発生することもあり得ます。
    ...
    def self.openapi_types
      {
        ...
        :'deposit_others' => :'Array<DepositOthers>',
        :'coupons' => :'Array<CouponsForTransactions>',
        :'coupon_items' => :'Array<CouponItemsForTransactions>',
        :'store' => :'Store',
        :'customer' => :'Customer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetTransactionId` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetTransactionId`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'deposit_others')
        if (value = attributes[:'deposit_others']).is_a?(Array)
          self.deposit_others = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if !@subtotal.nil? && @subtotal.to_s.length < 1
      ...
      if !@coupon_discount.nil? && @coupon_discount.to_s.length < 1
        invalid_properties.push('invalid value for "coupon_discount", the character length must be great than or equal to 1.')
      end

      if !@total.nil? && @total.to_s.length < 1
      ...
      if !@barcode.nil? && @barcode.to_s.length < 1
        invalid_properties.push('invalid value for "barcode", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@subtotal.nil? && @subtotal.to_s.length < 1
      ...
      return false if !@coupon_discount.nil? && @coupon_discount.to_s.length < 1
      return false if !@total.nil? && @total.to_s.length < 1
      ...
      return false if !@barcode.nil? && @barcode.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] coupon_discount Value to be assigned
    def coupon_discount=(coupon_discount)
      if !coupon_discount.nil? && coupon_discount.to_s.length < 1
        fail ArgumentError, 'invalid value for "coupon_discount", the character length must be great than or equal to 1.'
      end
    ...
    def barcode=(barcode)
      if !barcode.nil? && barcode.to_s.length < 1
        fail ArgumentError, 'invalid value for "barcode", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [transaction_head_id, transaction_date_time, transaction_head_division, cancel_division, unit_non_discountsubtotal, unit_discountsubtotal, unit_staff_discountsubtotal, unit_bargain_discountsubtotal, subtotal, subtotal_for_discount, subtotal_discount_price, subtotal_discount_rate, subtotal_discount_division, point_discount, coupon_discount, total, tax_include, tax_exclude, rounding_division, rounding_price, cash_total, credit_total, deposit, deposit_cash, deposit_credit, change, tip_cash, tip_credit, amount, return_amount, cost_total, sales_head_division, in_tax_sales_total, out_tax_sales_total, non_tax_sales_total, non_sales_target_total, non_sales_target_out_tax_total, non_sales_target_in_tax_total, non_sales_target_tax_free_total, non_sales_target_cost_total, non_sales_target_amount, non_sales_target_return_amount, new_point, spend_point, point, total_point, current_mile, earn_mile, total_mile, adjustment_mile, adjustment_mile_division, adjustment_mile_value, store_id, terminal_id, customer_id, terminal_tran_id, terminal_tran_date_time, sum_division, adjustment_date_time, sum_date, customer_rank, customer_group_id, customer_group_id2, customer_group_id3, customer_group_id4, customer_group_id5, staff_id, staff_name, credit_division, payment_count, slip_number, cancel_slip_number, auth_number, auth_date, card_company, denomination, memo, receipt_memo, carriage, commission, guest_numbers, guest_numbers_male, guest_numbers_female, guest_numbers_unknown, enter_date_time, tax_free_sales_division, net_tax_free_general_tax_include, net_tax_free_general_tax_exclude, net_tax_free_consumable_tax_include, net_tax_free_consumable_tax_exclude, tags, point_giving_division, point_giving_unit_price, point_giving_unit, point_spend_division, mileage_division, mileage_label, customer_pin_code, return_sales, dispose_division, dispose_server_transaction_head_id, cancel_date_time, sell_division, tax_rate, tax_rounding, discount_rounding_division, transaction_uuid, exchange_ticket_no, gift_receipt_valid_days, barcode, upd_date_time, details, deposit_others, coupons, coupon_items, store, customer, customer_groups, staff, layaway, layaway_pick_up].hash
    end
